...
result<-function(n,k,m,R,pi,alpha_1, alpha_2,lambda_1,lambda_2, PQ, NRep)
{
  ...
  while(h<Nrep+1)
  {
    ...
    ####Estimation
    MLE<-optim(c(pi,alpha_1,lambda_1,alpha_2,lambda_2),MLL,lower=c(0.01, 0.01,0.01,0.01,0.01), upper=c(0.99,6,6,6,6), method="L-BFGS-B", control=list(fnscale=-1))$par
    M_pi=MLE[1]
    M_Alpha_1=MLE[2]
    M_Alpha_2=MLE[4]
    M_Lambda_1=MLE[3]
    ...
    if((M_Alpha_1<6.5)&& (M_Alpha_2<6.5)&&(M_Lambda_1<5.5)&&(M_Lambda_2<5.5) && (M_Alpha_1>0.5)&& (M_Alpha_2>0.5)&&(M_Lambda_1>0.5)&&(M_Lambda_2>0.5))
    {
      MLE_pi[h]=M_pi
      ...
      PthQ[h]= MLE_pi[h]*(((1-(1-(1-0.5)^(1/k))^(1/MLE_Alpha_1[h]))^(-1/MLE_Lambda_1[h]))-1) + (1-MLE_pi[h])*(((1-(1-(1-0.5)^(1/k))^(1/MLE_Alpha_2[h]))^(-1/MLE_Lambda_2[h]))-1)
...
n<-60
m<-60
PQ<-(-(1/lambda_1)*log(1-(1-(1-0.5)^(1/k))^(1/alpha_1)))*pi+(-(1/lambda_2)*log(1-(1-(1-0.5)^(1/k))^(1/alpha_2)))*(1-pi)
#R<-c(n-m,rep(0,m-1))
#R<-c(rep(0,m-1),n-m)
