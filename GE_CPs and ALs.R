...
Nrep=1000
  ...
  while(h<Nrep+1)
  {
    ...
    ####Estimation
    MLE<-optim(c(pi,alpha_1,lambda_1,alpha_2,lambda_2),MLL_GE,lower=c(0.01, 0.01,0.01,0.01,0.01), upper=c(0.99,5,5,5,5), method="L-BFGS-B", control=list(fnscale=-1),hessian = TRUE)
    M_pi=MLE$par[1]
    M_Alpha_1=MLE$par[2]
    M_Alpha_2=MLE$par[4]
    ...
    if((M_pi<0.6)&&(M_Alpha_1<5.5)&& (M_Alpha_2<5.5)&&(M_Lambda_1<5.5)&&(M_Lambda_2<4.5) && (M_Alpha_1>0)&& (M_Alpha_2>0)&&(M_Lambda_1>0)&&(M_Lambda_2>0)&&(sd_one>0)&&(sd_two>0)&&(sd_three>0)&&(sd_four>0)&&(sd_five>0)&& (is.finite(sd_one)))
    {
      MLE_pi[h]=M_pi
